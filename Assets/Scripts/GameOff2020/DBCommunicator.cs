...
public static class DBCommunicator
{
    ...
    {
        RestClient.Put<User>($"{databaseURL}LastUID.json", uIData).Then(response => { callback(); });
    }
    
    public static void GetLastUserID(GetLastUserIdCallback callback)
	{
        RestClient.Get<LastUId>($"{databaseURL}LastUID.json").Then(response => { callback(response); });
    }
	...
	{
        RestClient.Get<UserDetails>($"{databaseURL}Users/{uID}.json").Then(response => { callback(response); });
    }
